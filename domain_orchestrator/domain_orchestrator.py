...
class DomainOrchestrator:
    def __init__(
        ...
        # sed_path = os.path.join(parent_dir, "sed") # TODO: This is a hardcoded path, it should be a parameter
        ...
        print(f"Changing directory to '{sed_path}' ...")

        try:
            os.chdir(sed_path)
        except FileNotFoundError:
            print(f"Error: The specified path '{sed_path}' does not exist.")
            exit(1)
        except PermissionError:
            print(f"Error: Insufficient permissions to access '{sed_path}'.")
            exit(1)
        except Exception as e:
            print(f"Unexpected error while changing directory: {e}")
            exit(1)
